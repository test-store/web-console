...
namespace FinalSite.Helper
{
    public class Util
    {
        ...
        public String Request(Object model, string url, string api_key, string type_method)
        {
            ...
            var clientToken = new RestClient(config.url_api_token);
        ...
        public String RequestToken(Object model, string url, string api_key, string type_method)
        {
            var clientToken = new RestClient(config.url_api_token);

            RestSharp.RestRequest request = new RestRequest();

           if (type_method.Equals("post"))
            {
                request = new RestRequest(url, Method.POST);
                request.AddJsonBody(model);
            }
